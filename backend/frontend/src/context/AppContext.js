...
export const AppProvider = ({ children }) => {
    ...
    const fetchCurrentPeriod = useCallback(async () => {
          try {
                  ...
                  const period = response.data.current_period || 'P1';
    ...
    useEffect(() => {
          const init = async () => {
                  ...
                  await seedOfficers();
                  await fetchOfficers();
                  const activePeriod = await fetchCurrentPeriod();
                  const days = ['thursday', 'friday', 'saturday', 'sunday'];
                  const types = ['rdo', 'days_ext', 'nights_ext'];
                  for (const day of days) {
                            for (const type of types) {
                                        await fetchSheet(day, type, activePeriod);
                            }
                  }
                  await fetchBumpedOfficers();
                  setLoading(false);
          ...
          init();
    }, [seedOfficers, fetchOfficers, fetchCurrentPeriod, fetchSheet, fetchBumpedOfficers]);
