...
async def export_sheet_pdf(period: str, day: str, sheet_type: str):
    ...
    for row in sheet.get('rows', []):
        a = row.get('assignment_a') or {}
        name = (a.get('officer_display') or '').split('—')[0].split('â')[0].split('-')[0].strip() if a.get('officer_display') else ''
...
async def generate_supervisor_log_pdf(
    ...
    for row in sheet.get("rows", []):
        ...
        name_part = raw.split('—')[0].split('â')[0].split('-')[0].strip().strip() if raw else ""
