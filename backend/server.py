        ...
        for sheet_type in SHEET_TYPES:
            ...
            for row in sheet.get('rows', []):
                ...
                if a.get('officer_id') == officer_id:
                    ...
                    if sheet_type == 'rdo': rdo += h
                    elif sheet_type == 'days_ext': days += h
                    elif sheet_type == 'nights_ext': nights += h
                    shifts += 1
    accum_id = f"{period}_{officer_id}"
    doc = {
        "accum_id": accum_id, "officer_id": officer_id,
        "officer_display": officer_display, "star": star, "period": period,
        "rdo_hours": rdo, "days_ext_hours": days, "nights_ext_hours": nights,
        "total_hours": rdo + days + nights, "shifts_worked": shifts,
...
async def get_officer_ytd(officer_id: str):
    ...
    return {
        ...
        "ytd_rdo_hours": sum(r.get('rdo_hours', 0) for r in records),
        "ytd_days_ext_hours": sum(r.get('days_ext_hours', 0) for r in records),
        "ytd_nights_ext_hours": sum(r.get('nights_ext_hours', 0) for r in records),
        "ytd_total_hours": sum(r.get('total_hours', 0) for r in records),
...
async def lock_sheet(period: str, day: str, sheet_type: str):
    ...
    await db.sheets.update_one({"sheet_id": sheet_id}, {"$set": {
        "locked": True, "locked_at": datetime.now(timezone.utc).isoformat(), "locked_by": "Admin"
    }})
...
async def set_auto_lock(period: str, day: str, sheet_type: str, request: AutoLockRequest):
    sheet_id = make_sheet_id(period, day, sheet_type)
    await db.sheets.update_one({"sheet_id": sheet_id}, {"$set": {
        "auto_lock_time": request.auto_lock_time, "auto_lock_enabled": request.auto_lock_enabled
    }})
...
async def export_sheet_pdf(period: str, day: str, sheet_type: str):
    ...
    for row in sheet.get('rows', []):
        a = row.get('assignment_a') or {}
        name = (a.get('officer_display') or '').encode('latin-1', errors='ignore').decode('utf-8', errors='ignore').split('—')[0].split('-')[0].strip() if a.get('officer_display') else ''
...
async def export_period_summary_pdf(period: str):
    ...
    elements.append(Paragraph(
        f"{period_dates.get('start','')} — {period_dates.get('end','')} &nbsp;&nbsp; Generated: {datetime.now(timezone.utc).strftime('%m/%d/%Y %H:%M')} UTC",
        styles['Normal']
    ))
    elements.append(Spacer(1, 12))
    data = [['Officer', 'Star', 'RDO Hrs', 'Days Ext', 'Nights Ext', 'Total Hrs', 'Shifts']]
    for r in records:
        data.append([r.get('officer_display',''), r.get('star',''),
...
# ==================== SUPERVISOR LOG PDF (Auto-fill CPD-11.455) ====================
@api_router.get("/sheets/{period}/{day}/{sheet_type}/supervisor-log-pdf")
async def generate_supervisor_log_pdf(
    period: str, day: str, sheet_type: str,
    sgt_name: str = "", sgt_star: str = ""
):
    """
    ...
    template_path = _os.path.join(
        _os.path.dirname(__file__), "frontend", "public", "Supervisors log blank.pdf"
    )
    if not _os.path.exists(template_path):
        raise HTTPException(status_code=404, detail="PDF template not found at: " + template_path)
    ...
    for row in sheet.get("rows", []):
        ...
        name_part = raw.encode('latin-1', errors='ignore').decode('utf-8', errors='ignore').split('—')[0].split('-')[0].strip().strip() if raw else ""
    ...
    filename = (
        f"SupervisorLog_214_{day_labels_upper.get(day, day.upper())}"
        f"_{sheet_type.upper()}_{log_date.replace('/','')}.pdf"
    )
